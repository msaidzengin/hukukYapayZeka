...
def find_similars(document_embeddings, text, top_k=5, threshold=0.5):
    ...
    result = []
    for idx in indices:
        result.append(float(pairwise_similarities[0][idx + 1]))
    return result

with open('kararlar.json', encoding='utf-8') as f:
    kararlar = json.load(f)

kararlar = [i for i in kararlar.values()][:10]
document_embeddings = create_vectors(kararlar)
...
print(find_similars(document_embeddings, text))